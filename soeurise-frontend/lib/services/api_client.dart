...
import '../src/read_file_bytes.dart';
...
class ApiClient {
  ...
  static (String type, String subtype) _mimeFromFilename(String filename) {
    ...
    String subtype = rawExt.isEmpty ? 'octet-stream' : rawExt;
    ...
    if (imageExts.contains(rawExt)) {
      ...
      type = 'application';
      subtype = rawExt.isEmpty ? 'octet-stream' : rawExt;
    }
    return (type, subtype);
  }

  /// Multipart upload (for avatars / images). Sur le web, passez [fileBytes] + [uploadFileName].
    ...
    try {
      ...
      if (fileField != null) {
        ...
        if (bytes != null && bytes.isNotEmpty) {
          final filename =
              (uploadFileName != null && uploadFileName.isNotEmpty)
                  ? uploadFileName
  ...
  Future<ApiResponse> multipartPost(
    ...
    return multipartRequest(
      ...
      path,
      fields: fields,
      fileField: hasFile || hasBytes ? fileField : null,
      filePath: hasFile ? file.path : null,
