...
class _PrivateChatScreenState extends State<PrivateChatScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    if (_pendingMedia != null) {
      ...
      msg = await _messageService.sendTextMessage(widget.user.id, text);
    }
  ...
  Widget _buildMessageBubble(PrivateMessage msg, bool isMe, bool isLastOutgoing) {
    ...
    return GestureDetector(
      onLongPress: isMe && !msg.deletedForAll ? () => _deleteMessage(msg) : null,
              ...
              child: Row(
                children: [
                  if (_pendingMediaType == 'image')
                    ClipRRect(
                      borderRadius: BorderRadius.circular(12),
                      child: Image.file(_pendingMedia!, width: 64, height: 64, fit: BoxFit.cover),
                    )
                  else
