...
import '../widgets/local_path_circle_avatar.dart';
...
class HomeScreenState extends State<HomeScreen>
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 16),
                        Text('Publication de votre story...', style: TextStyle(fontWeight: FontWeight.bold)),
                        SizedBox(height: 8),
  ...
  Future<void> _createStory() async {
    ...
    // Check permissions
    if (!kIsWeb) {
      final status = await (pickedType == 'image'
    ...
    setState(() => _isStoryUploading = true);
    Story? created;
    try {
      if (kIsWeb) {
        final bytes = await file.readAsBytes();
        final name = file.name.trim();
        final fallback = pickedType == 'image' ? 'story.jpg' : 'story.mp4';
        created = await StoryService.instance.createStory(
          mediaBytes: bytes,
          mediaFileName: name.isNotEmpty ? name : fallback,
      ...
      if (mounted) setState(() => _isStoryUploading = false);
    }
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final Gradient ringGradient = LinearGradient(
                colors: [
                  isViewed
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        border: Border.all(
                          color: isViewed ? AppColors.beigeDark : AppColors.primary,
                          width: 2,
                        ),
                      ),
                      child: UserAvatar(
                        ...
                        child: Column(
                          children: [
                            Container(
                              ...
                              child: (profile.profileImageLocalPath != null &&
                                      profile.profileImageLocalPath!
                                          .trim()
                                          .isNotEmpty)
                                  ? localPathCircleAvatar(
                                      path: profile.profileImageLocalPath!,
                                      radius: 44,
                                    )
                                  : UserAvatar(
                                      imageUrl:
