...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  StreamSubscription? _deletedMsgSubscription;
  StreamSubscription? _readMsgSubscription;
  StreamSubscription? _editedMsgSubscription;
  StreamSubscription? _reactedMsgSubscription;
  bool _isAdmin = false;
  ...
  void initState() {
    ...
    _deletedMsgSubscription = SocketService().groupMessageDeletedStream?.listen((data) {
      final gid = data['groupId']?.toString() ?? '';
      final mid = data['messageId']?.toString() ?? '';
      if (gid == widget.communityId && mid.isNotEmpty) {
        CommunityService.instance.removeMessageFromSocket(widget.communityId, mid);
      }
    });
    // Listen for group messages read by others
    _readMsgSubscription = SocketService().groupMessageReadStream?.listen((data) {
      if (data['groupId'] == widget.communityId) {
        // Just reload messages to get updated seenBy
        _loadMessages(markRead: false);
      }
    });

    _editedMsgSubscription = SocketService().groupMessageEditedStream?.listen((data) {
      if (data['groupId'] == widget.communityId) {
        CommunityService.instance.updateMessageFromSocket(widget.communityId, data);
      }
    });

    _reactedMsgSubscription = SocketService().groupMessageReactedStream?.listen((data) {
      if (data['groupId'] == widget.communityId) {
        CommunityService.instance.updateMessageFromSocket(widget.communityId, data);
      }
    });
  ...
  Future<void> _loadRole() async {
    final role = await CommunityService.instance.getMyRole(widget.communityId);
    if (mounted) {
      setState(() {
  ...
  Future<void> _loadMessages({bool markRead = true}) async {
    ...
    if (markRead) {
      await CommunityService.instance.markMessagesRead(widget.communityId);
    }
  }
  ...
  void dispose() {
    ...
    _deletedMsgSubscription?.cancel();
    _readMsgSubscription?.cancel();
    _editedMsgSubscription?.cancel();
    _reactedMsgSubscription?.cancel();
    _controller.dispose();
    _recorder.dispose();
  ...
  Future<void> _sendMessage() async {
    ...
    if (_editingMessage != null) {
      final msg = await CommunityService.instance.editGroupMessage(widget.communityId, _editingMessage!.id, text);
      ok = msg != null;
    } else {
  ...
  void _showDeleteMenu(ChatMessage msg) {
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Options du message', style: AppTextStyles.headline4),
            ...
            if (msg.senderId == ProfileService.instance.profile.value.id && msg.type == 'text')
              ListTile(
            ...
            if (_isAdmin || msg.senderId == ProfileService.instance.profile.value.id)
              ListTile(
                ...
                onTap: () async {
                  Navigator.pop(context);
                  final ok = await CommunityService.instance.deleteGroupMessage(
                    widget.communityId,
                    msg.id,
                  );
                  if (!mounted) return;
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Wrap(
          ...
          children: emojis.map((e) => GestureDetector(
            onTap: () async {
              Navigator.pop(ctx);
              final updated = await CommunityService.instance.reactToGroupMessage(widget.communityId, msg.id, e);
              if (updated != null && mounted) {
                CommunityService.instance.updateMessageFromSocket(widget.communityId, updated.toJson());
              }
            },
